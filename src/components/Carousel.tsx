...
const Carousel: React.FC = () => {
  ...
  return (
    <div className="flex flex-col justify-evenly mt-10 bg-slate--400 w-[100%] h-[100vh]">
      ...
      <div>
        <div className="flex justify-start items-center">
          <button onClick={showPreviousImages} disabled={currentIndex === 0} className='flex justify-center items-center absolute z-[5] rounded-[50%] 
			            border-1 border-solid transition-[0.25]  top-[65rem]  p-0 cursor-pointer bg-white'>
            <GrFormPrevious size={`40px`} />
          </button>
        </div>

        <div className="flex justify-between w-full h-[70vh] relative overflow-hidden z-0">

          {Nfts.slice(currentIndex, currentIndex + 4).map((image, index) => (
            <img key={index} src={image.imgUrl} className="w-[287px] rounded-xl object-cover h-full  transition  bg-blue-200" alt={`Image ${currentIndex + index}`} />
          ))}
        ...
        <div className="flex justify-end items-center bg-red-500">
          ...
          <button onClick={showNextImages} disabled={currentIndex + 4 >= Nfts.length} className='flex justify-center items-center absolute z-[5] rounded-[50%] 
              border-1 border-solid transition-[0.25] top-[65rem] p-0 cursor-pointer bg-white'>
            <GrFormNext size={`40px`} className='' />
          </button>
        </div>
      </div>
